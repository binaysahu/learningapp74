        ...
        "SELECT CourseID,CourseName,Rating FROM Course;",sqlConnection);
         using (SqlDataReader sqlDatareader = sqlcommand.ExecuteReader())
         {
             while (sqlDatareader.Read())
                {
                    Courses.Add(new Course() {CourseID=Int32.TryParse(sqlDatareader["CourseID"].ToString(), out int result) ? result : 0,
                    CourseName=sqlDatareader["CourseName"].ToString(),
                    Rating=Decimal.TryParse(sqlDatareader["Rating"].ToString(), out decimal rating) ? rating : 0});
                }
         }
    }
}
